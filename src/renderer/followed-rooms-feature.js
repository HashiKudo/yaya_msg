(function () {
  ...
  window.YayaRendererFeatures.createFollowedRoomsFeature =
    function createFollowedRoomsFeature(deps) {
      const {
      ...
      function selectFollowedSort(value, text) {
        ...
        document.getElementById("followed-sort-dropdown").style.display =
          "none";
        sortFollowedRooms();
      }
      ...
      function compareFollowedNames(a, b) {
        if (window.pinyinPro) {
          const pA = pinyinPro
            .pinyin(a.bigDisplayName, { toneType: "none", type: "array" })
            .join("");
          const pB = pinyinPro
            .pinyin(b.bigDisplayName, { toneType: "none", type: "array" })
            .join("");
      ...
      function filterFollowedRoomsByKeyword(data) {
        ...
        return data.filter((item) => {
          ...
          const namePinyin = getFollowedSearchPinyinParts(
            item.bigDisplayName || item.ownerName || item.pinkStarName,
          );
      ...
      function sortFollowedRooms() {
        const sortMode =
          document.getElementById("followed-sort-value")?.value || "default";
        ...
        if (sortMode === "name") {
          sortedData.sort(
            (a, b) =>
              comparePinnedState(a, b) ||
              compareInactiveState(a, b) ||
              compareFollowedNames(a, b),
          );
          ...
          if (savedOrder.length > 0) {
            ...
            sortedData.sort(
              (a, b) => comparePinnedState(a, b) || b.msgTime - a.msgTime,
            );
          }
        }
      ...
      function writeSavedCustomOrder(order) {
        writeJsonSetting(FOLLOWED_CUSTOM_ORDER_KEY, [
          ...new Set(order.map((id) => String(id)).filter(Boolean)),
        ]);
      }

      function removeChannelFromCustomOrder(channelId) {
        const targetChannelId = String(channelId || "");
        writeSavedCustomOrder(
          getSavedCustomOrder().filter((id) => id !== targetChannelId),
        );
        writePinnedChannelIds(
          getPinnedChannelIds().filter((id) => id !== targetChannelId),
        );
      }

      function getPinnedChannelIds() {
        const pinnedIds = readJsonSetting(FOLLOWED_PINNED_CHANNELS_KEY, []);
        return Array.isArray(pinnedIds)
          ? pinnedIds.map((id) => String(id))
          : [];
      }

      function writePinnedChannelIds(channelIds) {
        writeJsonSetting(FOLLOWED_PINNED_CHANNELS_KEY, [
          ...new Set(channelIds.map((id) => String(id)).filter(Boolean)),
        ]);
      }
      ...
      async function loadFollowedRooms(options = {}) {
        ...
        if (!token) {
          container.innerHTML =
            '<div class="placeholder-tip"><h3>未登录</h3></div>';
          return;
        }
        ...
        try {
          ...
          if (friendsRes.status !== 200 || !friendsRes.content?.data)
          ...
          followedIds.forEach((uid) => {
            const member = memberData.find(
              (m) => String(m.id || m.userId) === String(uid),
            );
          ...
          currentFollowedData = followedMembers.map((m) => {
            const msg =
              lastMsgs.find(
                (msg) => String(msg.channelId) === String(m.channelId),
              ) || {};
            const apiStarName = msg.starName || m.ownerName || "未知";
            let bigDisplayName = m.ownerName || "未知成员";

            if (
              bigDisplayName.includes("SNH48-") ||
              bigDisplayName.includes("GNZ48-") ||
              bigDisplayName.includes("BEJ48-") ||
              bigDisplayName.includes("CKG48-") ||
      ...
      function resetFollowedRoomsState() {
        ...
        if (container) {
          container.innerHTML =
            '<div class="empty-state" style="margin-top: 50px;">正在加载房间列表</div>';
        }
      ...
      function startFollowedRoomsPolling() {
        ...
        const scheduleNext = () => {
          ...
          followedRoomsAutoRefreshTimer = setTimeout(
            runPoll,
        ...
        const runPoll = async () => {
          ...
          if (
            ...
            followedRoomsAutoRefreshRunning
          ) {
            scheduleNext();
            return;
          }
      ...
      function renderFollowedRoomsList(renderData) {
        ...
        const html = renderData
          .map((item, index) => {
            ...
            const shouldRenderPinnedDivider =
              previousItem &&
            ...
            const isActive =
              String(getActiveFollowedChannel()) === String(item.channelId)
                ? "active"
                : "";
            const draggableAttr = isCustomSort ? 'draggable="true"' : "";
            const cursorStyle = isCustomSort
              ? "cursor: grab;"
              : "cursor: pointer;";
    ...
    `;
      ...
      function saveCustomRoomOrder() {
        ...
        const newOrder = Array.from(cards).map((card) =>
          card.getAttribute("data-channelid"),
        );
      ...
      function toggleFollowedRoomPinned(card) {
        ...
        const visibleOrder = Array.from(
          document.querySelectorAll("#followed-rooms-container .session-card"),
        )
      ...
      function unfollowFromFollowedRoomCard(card) {
        ...
        showFollowedRoomConfirm(`确定取关 ${memberName} 吗？`, async () => {
          ...
          try {
            ...
            currentFollowedData = currentFollowedData.filter(
              (item) => String(item.id || item.userId) !== String(memberId),
            );
            removeChannelFromCustomOrder(channelId);
            sortFollowedRooms();
            showToast(`已取消关注 ${memberName}`);
            setTimeout(
              () => loadFollowedRooms({ silent: true, preserveScroll: true }),
              500,
            );
          } catch (error) {
        ...
        if (matches.length > 0) {
          ...
          resultBox.querySelectorAll(".suggestion-item").forEach((item) => {
            item.addEventListener("click", () =>
              selectQuickFollowMember(item.dataset.name, item.dataset.id),
            );
          });
          resultBox.style.display = "block";
        } else {
          resultBox.innerHTML =
            '<div class="suggestion-item" style="font-size:12px; color:#999;">未找到该成员</div>';
          resultBox.style.display = "block";
        }
      }
      ...
      async function executeQuickAction() {
        ...
        const isUnfollow =
          window.allFollowedIds.has(String(memberId)) ||
          btn.innerText === "取关";
        ...
        try {
          ...
          if (res.success) {
            showToast(
              `${isUnfollow ? "已取消关注" : "成功关注"} ${memberName}`,
            );

            if (isUnfollow) {
              const removed = currentFollowedData.find(
                (item) => String(item.id || item.userId) === String(memberId),
              );
              window.allFollowedIds.delete(String(memberId));
              currentFollowedData = currentFollowedData.filter(
