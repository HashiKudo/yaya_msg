...
function setMeet48LoginStatus(message, type = 'muted') {
    ...
    statusEl.style.color = type === 'success' ? '#28a745' : type === 'error' ? '#ff4d4f' : 'var(--text-sub)';
}
...
async function validateMeet48AuthSaved() {
    ...
    const firstItem = normalizeMeet48LiveList(listResult.content).find((item) => item && (item.liveId || item.id));
    if (!firstItem) {
...
function renderAccountRenameCount(content) {
    ...
    const normalized = content === undefined || content === null || content === '' ? '--' : String(content);
...
function renderAccountChickenBalance(content) {
    ...
    if (!balanceEl) return;
    const total = content && typeof content === 'object' ? (content.moneyTotal ?? content.total ?? content.balance ?? content.money) : content;
...
function ensureAutoCheckinSettingsMigrated() {
    ...
    const settingsApi = getAppSettingsApi();
    if (!settingsApi || typeof settingsApi.getSettingValueSync !== 'function' || typeof settingsApi.setSettingValueSync !== 'function') {
        return;
    }
...
function buildFetchMessageKey(message) {
    ...
    let bodySeed = '';
    try {
        bodySeed = typeof message.bodys === 'string' ? message.bodys : JSON.stringify(message.bodys || message.msgContent || '');
    } catch (error) {
...
function normalizeAutoMessageFetchConfig(value) {
    ...
    return {
        ...
        intervalMinutes: allowedIntervals.has(intervalMinutes) ? intervalMinutes : AUTO_MESSAGE_FETCH_DEFAULT_INTERVAL,
        members,
...
function handleAutoFetchMemberSearch(keyword) {
    ...
    const source = typeof memberData !== 'undefined' && Array.isArray(memberData) ? memberData : Array.isArray(window.memberData) ? window.memberData : [];
...
function removeAutoMessageFetchMember(serverId, channelId) {
    const config = getAutoMessageFetchDraftConfig(true);
    config.members = config.members.filter((member) => member.serverId !== String(serverId || '') || member.channelId !== String(channelId || ''));
    if (config.members.length === 0) {
...
function saveAutoMessageFetchSettings(options = {}) {
    ...
    const scheduleTimingChanged =
        draftConfig.scheduledEnabled && (!previousConfig.scheduledEnabled || previousConfig.intervalMinutes !== draftConfig.intervalMinutes);
    ...
    closeAutoMessageFetchDropdowns();
    renderAutoMessageFetchUi();
    const missingSmallRoomCount =
        savedConfig.roomType === 'small' || savedConfig.roomType === 'all' ? savedConfig.members.filter((member) => !member.smallChannelId).length : 0;
    if (missingSmallRoomCount > 0) {
        const missingMessage =
            savedConfig.roomType === 'all' ? `${missingSmallRoomCount} 位成员缺少小房间参数，将只抓取大房间` : `${missingSmallRoomCount} 位成员缺少小房间参数`;
...
function isCheckinRecordedToday(userId) {
    ...
    return (
        String(readAutoCheckinSetting(AUTO_CHECKIN_LAST_DATE_KEY, '')) === getTodayCheckinKey() &&
...
async function fetchMsgs(isLoadMore = false) {
    ...
    `;
                    ...
                    const displayName =
                        m.senderName || (m.extInfo && (typeof m.extInfo === 'string' ? JSON.parse(m.extInfo) : m.extInfo).user.nickName) || '未知用户';
                    const idDisplay =
                        senderId && String(senderId) !== '0' ? `<span style="color:#999; font-size:10px; margin-left:6px; ">(ID:${senderId})</span>` : '';
...
function getFetchExportMemberMeta(memberName, channelId) {
    ...
    const members = typeof memberData !== 'undefined' && Array.isArray(memberData) ? memberData : Array.isArray(window.memberData) ? window.memberData : [];

    const member = members.find((m) => {
        const names = [m.ownerName, m.name, m.nickname, m.nickName, m.memberName].map((value) => String(value || '').trim());
        const channelIds = [m.channelId, m.roomId, m.channel_id, m.yklzId, m.smallRoomId, m.smallChannelId].map((value) => String(value || '').trim());

        return (targetChannelId && channelIds.includes(targetChannelId)) || (targetName && names.includes(targetName));
    });

    const resolvedMemberName = String(
        (member && (member.ownerName || member.name || member.nickname || member.nickName || member.memberName)) || targetName || '未命名成员'
    ).trim();
    const groupName = String((member && (member.groupName || member.teamName || member.group || member.team)) || '未知分团').trim();
...
function buildFetchExportKey(message, exportUserId, rawBody) {
    ...
    const seed = [primaryId, message.msgTime || '', message.msgType || '', exportUserId || '', normalizedBody].join('|');
...
function getActiveMaskwordElements() {
    ...
    const input =
        document.getElementById(isPbcVisible ? 'pbc-maskword-input' : 'maskword-input') ||
        document.getElementById('maskword-input') ||
        document.getElementById('pbc-maskword-input');
    const result =
        document.getElementById(isPbcVisible ? 'pbc-maskword-result' : 'maskword-result') ||
...
async function updateMaskwordLibrary() {
    ...
    try {
        ...
        maskwordLibrary = Array.isArray(content.words) ? content.words.map((word) => String(word || '').trim()).filter(Boolean) : [];
