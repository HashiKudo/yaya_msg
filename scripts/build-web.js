...
const buildDatabaseRuntimeOnly = process.argv.includes('--database-runtime-only');
...
function bundleWebRendererScripts(indexHtml) {
    const startMarker = `<script defer src="./src/renderer/bootstrap-shared.js?v=${buildVersion}"></script>`;
    const endMarker = `<script defer src="./src/renderer/app-legacy.js?v=${buildVersion}"></script>`;
    ...
    const scriptBlock = indexHtml.slice(startIndex, endIndex);
    const rendererPaths = Array.from(
        scriptBlock.matchAll(/src="\.\/(src\/renderer\/[^"?]+\.js)\?v=[^"]+"/g),
        (match) => match[1]
    );
    if (rendererPaths.length < 2
        || rendererPaths[0] !== 'src/renderer/bootstrap-shared.js'
        || rendererPaths[rendererPaths.length - 1] !== 'src/renderer/app-legacy.js') {
        throw new Error('Web renderer bundle script order is invalid');
    }

    const bundleRelativePath = 'src/renderer/web-app-bundle.js';
    const bundlePath = path.join(outputDir, bundleRelativePath);
    const bundleSource = rendererPaths.map((relativePath) => {
        const source = fs.readFileSync(path.join(outputDir, relativePath), 'utf8');
        return `// ${relativePath}\n${source.trim()}\n;`;
    }).join('\n\n');
    fs.writeFileSync(
        bundlePath,
    ...
    const bundleTag = `<script defer src="./${bundleRelativePath}?v=${buildVersion}"></script>`;
...
function createWebTopbarHtml() {
    return [
        ...
        '            <button class="web-topbar-btn web-pbc-btn" type="button" onclick="openWebHomeRoute(\'pbc\')" aria-label="屏蔽词检测" title="屏蔽词检测">屏蔽词检测</button>',
...
async function buildDatabaseRuntime(sourcePath, runtimePath, banner) {
    ...
    const databaseSource = rewriteDatabaseImports(source).replace(
...
async function buildWebDatabaseRuntime(databaseSourcePath) {
...
async function buildDesktopDatabaseRuntime() {
    await buildDatabaseRuntime(
...
async function applyWebTransforms() {
    ...
    const sdkInitPath = path.join(outputDir, 'src', 'renderer', 'sdk-init.js');
    const databaseSourcePath = path.join(outputDir, 'src', 'renderer', 'database', 'app.jsx');
    ...
    let sdkInit = fs.readFileSync(sdkInitPath, 'utf8');
    await buildWebDatabaseRuntime(databaseSourcePath);
    ...
    sdkInit = replaceOnce(
        sdkInit,
        "from '../../rust-wasm-browser.js'",
        `from '../../rust-wasm-browser.js?v=${buildVersion}'`
    );
    sdkInit = replaceOnce(
        sdkInit,
        "new URL('../../2.wasm', import.meta.url)",
        `new URL('../../2.wasm?v=${buildVersion}', import.meta.url)`
    );
    ...
    indexHtml = bundleWebRendererScripts(fs.readFileSync(indexPath, 'utf8'));
    fs.writeFileSync(indexPath, indexHtml);
...
if (buildDatabaseRuntimeOnly) {
    buildDesktopDatabaseRuntime()
        .then(() => {
            console.log('Database runtime built');
        })
        .catch((error) => {
            console.error(error);
            process.exitCode = 1;
